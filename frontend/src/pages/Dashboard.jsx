      ...
      <main className="flex-1 mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex space-x-3">
            <button data-testid="dashboard-manage-category-button" onClick={() => handleToggleCategoriesModal(true)} className="rounded border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
              Manage Categories
            </button>
            <button data-testid="dashboard-add-task-button" onClick={() => { setTaskToEdit(null); setIsTaskModalOpen(true); }} className="rounded bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors">
              + New Task
            </button>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 rounded-lg bg-white p-4 shadow-sm border border-gray-200">
          <div className="flex-1 relative">
            <input data-testid="dashboard-search-input" type="text" placeholder="Search tasks..." className="w-full rounded-md border border-gray-300 pl-10 pr-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchFilteredTasks()} />
          ...
          <div className="w-full sm:w-64">
            <select data-testid="dashboard-search-category-button" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 bg-white" value={filterCategory} onChange={(e) => { setFilterCategory(e.target.value); fetchFilteredTasks(searchQuery, e.target.value); }}>
              <option value="">All Categories</option>
              {categories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
            </select>
          </div>
          <button data-testid="dashboard-search-button" onClick={() => fetchFilteredTasks()} className="rounded-md bg-gray-100 px-6 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200 border border-gray-300">Search</button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="flex flex-col bg-white rounded-lg shadow border border-gray-200 min-h-[430px] h-[calc(100vh-300px)]">
              ...
              <div className="shrink-0">
                <Pagination dataTestId="pending-tasks" page={pendingPage} hasNext={pendingHasNext} hasPrev={pendingHasPrev} onNext={nextPendingPage} onPrev={prevPendingPage} />
              </div>
            </div>
            ...
            <div className="flex flex-col bg-white rounded-lg shadow border border-gray-200 min-h-[430px] h-[calc(100vh-300px)]">
              ...
              <div className="shrink-0">
                <Pagination dataTestId="completed-tasks" page={completedPage} hasNext={completedHasNext} hasPrev={completedHasPrev} onNext={nextCompletedPage} onPrev={prevCompletedPage} />
              </div>
            </div>
