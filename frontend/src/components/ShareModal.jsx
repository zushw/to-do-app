    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button data-testid="close-share-modal-button" onClick={() => { onClose(); setMessage({type:'', text:''}); }} className="text-gray-400 hover:text-gray-600">
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <p data-testid="share-title" className="mb-4 text-sm text-gray-600 border-b pb-4">
        ...
        <div className="mb-6">
          ...
          {task.shared_with_usernames?.length === 0 ? (
            ...
            <ul data-testid="share-currently-users-list" className="divide-y divide-gray-100 rounded-md border border-gray-200">
              {task.shared_with_usernames?.map(username => (
                <li key={username} className="flex items-center justify-between p-3 text-sm">
                  <div className="flex items-center space-x-2">
                    ...
                    <span data-testid={`share-currently-user-username-${username}`} className="font-medium text-gray-700">{username}</span>
                  </div>
                  <button
                    onClick={() => handleUnshare(username)}
                    data-testid={`share-currently-user-remove-button-${username}`}
                    disabled={isLoading}
